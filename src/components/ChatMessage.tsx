...
function ChatMessage({ role, content, isStreaming }: ChatMessageProps) {
  ...
  if (isUser) {
    return (
      <div className="flex gap-3 justify-end items-end">
        <div className="max-w-xs px-4 py-2.5 rounded-2xl rounded-br-none text-sm leading-relaxed bg-gradient-to-r from-violet-600 to-blue-600 text-white whitespace-pre-wrap">
          {content}
        </div>
        <div className="w-8 h-8 rounded-full bg-violet-500/20 flex items-center justify-center flex-shrink-0 mb-1">
  ...
  const textContent = parsedContent?.artifacts
    .filter(artifact => artifact.type === "text" && !ClaraResponseParser.isHTML(artifact.content))
        ...
        {isStreaming && !textContent && parsedContent?.htmlCount === 0 && (
          <div className="text-xs text-slate-400 flex items-center gap-2">
            <div className="flex gap-1">
              <div className="w-1 h-1 bg-violet-500 rounded-full animate-bounce" style={{ animationDelay: "0ms" }} />
              <div className="w-1 h-1 bg-violet-500 rounded-full animate-bounce" style={{ animationDelay: "150ms" }} />
              <div className="w-1 h-1 bg-violet-500 rounded-full animate-bounce" style={{ animationDelay: "300ms" }} />
            </div>
